...
struct CD2DScrollStyle
{
	...
	{
		// 轨道：浅灰色背景，无边框，无圆角
		track.background.m_type = CD2DBackground::Solid;
		track.background.m_solid.color = D2D1::ColorF(0.9f, 0.9f, 0.9f);
		track.radius = CD2DRadius(4.0f);

		// 滑块：深灰色背景，小圆角（4px），无边框
		thumb.background.m_type = CD2DBackground::Solid;
		thumb.background.m_solid.color = D2D1::ColorF(0.6f, 0.6f, 0.6f);
