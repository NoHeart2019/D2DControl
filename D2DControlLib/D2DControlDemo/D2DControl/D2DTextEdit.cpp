...
void CD2DTextEdit::DrawControl(CD2DRender* pRender)
{
	...
	{
		// 准备文本样式（原始样式，不影响布局）
		CD2DTextStyle layoutStyle = style.text;
		layoutStyle.wordWrap = (m_mode == SingleLine)? false : true;
		// 计算布局高度 
		float layoutHeight = (m_mode == SingleLine) 
			? (textRect.bottom - textRect.top) 
			: ((layoutStyle.verticalAlign != DWRITE_PARAGRAPH_ALIGNMENT_NEAR) 
			? (textRect.bottom - textRect.top) 
			: FLT_MAX);

		// 直接使用 CD2DRender 的统一布局创建方法
		CComPtr<IDWriteTextLayout> spLayout = pRender->CreateTextLayout(
			showPlaceholder ? m_placeholderText : m_text,
			layoutStyle,
		...
		if (spLayout)
		{
			...
			// 绘制文本
			D2D1_COLOR_F textColor = showPlaceholder ? D2D1::ColorF(0.7f, 0.7f, 0.7f, 0.8f) : layoutStyle.color;
			CComPtr<ID2D1SolidColorBrush> spBrush;
			pRT->CreateSolidColorBrush(textColor, &spBrush);
			if (spBrush)
			{
				pRT->DrawTextLayout(D2D1::Point2F(textRect.left, contentRect.top), spLayout, spBrush,
					D2D1_DRAW_TEXT_OPTIONS_CLIP);
			}

			// 绘制光标（传入与文本完全相同的布局）
			if (!showPlaceholder && m_caretVisible && !m_readOnly)
			{
				DrawCaret(pRender, textRect, spLayout);
			}
	...
	IDWriteTextLayout* pLayout)
{
	...
	float caretX, caretY;
	pLayout->HitTestTextPosition(m_caretPos, FALSE, &caretX, &caretY, &metrics);
