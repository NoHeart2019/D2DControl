...
CD2DRender::CD2DRender() : m_dpiScaleX(1.0f), m_dpiScaleY(1.0f) , m_pCompatibleRenderTarget(nullptr){Initialize();}
...
void CD2DRender::GetDpiScale(float* dpiSacleX , float* dpiScaleY)
{ *dpiSacleX = m_dpiScaleX; *dpiScaleY = m_dpiScaleY ;}
...
CComPtr<ID2D1PathGeometry> CD2DRender::CreateRoundRectGeometry(const D2D1_RECT_F& rect, const CD2DRadius& radius)
{
	...
	// 计算关键点（同上）
	D2D1_POINT_2F p0 = D2D1::Point2F(rect.left + radius.topLeft, rect.top);
	D2D1_POINT_2F p1 = D2D1::Point2F(rect.right - radius.topRight, rect.top);
	D2D1_POINT_2F p2 = D2D1::Point2F(rect.right, rect.top + radius.topRight);
	D2D1_POINT_2F p3 = D2D1::Point2F(rect.right, rect.bottom - radius.bottomRight);
	D2D1_POINT_2F p4 = D2D1::Point2F(rect.right - radius.bottomRight, rect.bottom);
	D2D1_POINT_2F p5 = D2D1::Point2F(rect.left + radius.bottomLeft, rect.bottom);
	D2D1_POINT_2F p6 = D2D1::Point2F(rect.left, rect.bottom - radius.bottomLeft);
	D2D1_POINT_2F p7 = D2D1::Point2F(rect.left, rect.top + radius.topLeft);

	spSink->BeginFigure(p0, D2D1_FIGURE_BEGIN_FILLED);
	spSink->AddLine(p1);
	if (radius.topRight > 0)
		spSink->AddArc(D2D1::ArcSegment(p2, D2D1::SizeF(radius.topRight, radius.topRight), 0, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_SMALL));
	spSink->AddLine(p3);
	if (radius.bottomRight > 0)
		spSink->AddArc(D2D1::ArcSegment(p4, D2D1::SizeF(radius.bottomRight, radius.bottomRight), 0, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_SMALL));
	spSink->AddLine(p5);
	if (radius.bottomLeft > 0)
		spSink->AddArc(D2D1::ArcSegment(p6, D2D1::SizeF(radius.bottomLeft, radius.bottomLeft), 0, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_SMALL));
	spSink->AddLine(p7);
	if (radius.topLeft > 0)
		spSink->AddArc(D2D1::ArcSegment(p0, D2D1::SizeF(radius.topLeft, radius.topLeft), 0, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_SMALL));
	...
	case CD2DBackground::Solid:
		{
			...
			pRT->CreateSolidColorBrush(bg.solid.color, &spBrush);
	...
	case CD2DBackground::LinearGradient:
		{
			...
			// 根据弧度计算方向向量（0 弧度 = 正南方向，正角沿屏幕顺时针） CSS deg :bg.linear.degrees
			float radians = (bg.linear.degrees - 90.0f) * 3.14159265359f  / 180.0f;
			...
			for (int i = 0; i < bg.linear.stopCount; ++i)
			{
				stops[i].position = bg.linear.stops[i].position;
				stops[i].color = bg.linear.stops[i].color;
			}
			pRT->CreateGradientStopCollection(
				stops, bg.linear.stopCount, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, &spStops);
			if (!spStops)
				return nullptr;
	...
	case CD2DBackground::RadialGradient:
		{
			CComPtr<ID2D1GradientStopCollection> spStops;
			D2D1_GRADIENT_STOP stops[10];
			for (int i = 0; i < bg.radial.stopCount; ++i)
			{
				stops[i].position = bg.radial.stops[i].position;
				stops[i].color = bg.radial.stops[i].color;
			}
			pRT->CreateGradientStopCollection(
				stops, bg.radial.stopCount, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, &spStops);
			if (!spStops)
				return nullptr;
			...
			pRT->CreateRadialGradientBrush(
				...
				bg.radial.center,
				bg.radial.gradientOriginOffset,
				bg.radial.radiusX,
				bg.radial.radiusY),
				spStops,
				&spBrush);
...
CComPtr<IDWriteTextFormat> CD2DRender::CreateTextFormat(const CD2DTextStyle& style)
{
	// 构建缓存键
	CAtlString key;
	key.Format(L"%s|%.1f|%d|%d|%d|%d|%d|%d",
		style.fontFamily, style.fontSize,
		style.fontWeight, style.fontStyle, style.fontStretch, style.horizontalAlign, style.verticalAlign, style.wordWrap);
...
CComPtr<IDWriteTextLayout> CD2DRender::CreateTextLayout(const CAtlString& text, const CD2DTextStyle& style,
	float maxWidth, float maxHeight) const
{
	if (text.IsEmpty()) return nullptr;

	CComPtr<IDWriteTextFormat> spFormat = const_cast<CD2DRender*>(this)->CreateTextFormat(style);
	if (!spFormat) return nullptr;

	IDWriteFactory* pDW = GetWriteFactory();
	if (!pDW) return nullptr;
	...
	// 设置行高（若样式中有效）
	if (style.lineHeight != 1.0f)
	{
		float lineH = CalculateActualLineHeight(style);
	...
	float maxWidth, float maxHeight) const
{
	...
	CComPtr<IDWriteTextLayout> spLayout = CreateTextLayout(text, textStyle, maxWidth, maxHeight);
	if (!spLayout) return D2D1::SizeF(0.0f, 0.0f);

	DWRITE_TEXT_METRICS metrics = {};
	if (SUCCEEDED(spLayout->GetMetrics(&metrics)))
		return D2D1::SizeF(metrics.width, metrics.height);
	...
	const D2D1_RECT_F& rect, float scrollOffsetY)
{
	...
	if (spBrush)
	{
		//pRT->DrawRectangle(rect, spBrush);
		// 直接以矩形左上角为原点，加上滚动偏移
		pRT->DrawTextLayout(
...
void CD2DControlBaseUI::OnDragMove(const D2D1_POINT_2F& delta)
{
	...
	m_posInfo.left += delta.x;
	m_posInfo.top += delta.y;
...
{
	return bg.type != CD2DBackground::None;
}
...
float GetStyleDecorationLeft(const CD2DStyle& style) {
	return style.margin.left + style.border.left.width + style.padding.left;
}

float GetStyleDecorationTop(const CD2DStyle& style) {
	return style.margin.top + style.border.top.width + style.padding.top;
}

float GetStyleDecorationRight(const CD2DStyle& style) {
	return style.margin.right + style.border.right.width + style.padding.right;
}

float GetStyleDecorationBottom(const CD2DStyle& style) {
	return style.margin.bottom + style.border.bottom.width + style.padding.bottom;
}
...
D2D1_RECT_F GetStyleDecoration(const CD2DStyle& style) {
	return D2D1::RectF(
		GetStyleDecorationLeft(style),
		GetStyleDecorationTop(style),
		GetStyleDecorationRight(style),
		GetStyleDecorationBottom(style)
		);
}
...
{
	return D2D1::SizeF(
		GetStyleDecorationLeft(style) + GetStyleDecorationRight(style),
		GetStyleDecorationTop(style)  + GetStyleDecorationBottom(style)
		);
}
...
void CD2DControlUI::ClearAllStyle()
{
	m_styles.RemoveAll();
}
...
{
	if (IsBorderEdgeSet(src.border.top))    dest.border.top    = src.border.top;
	if (IsBorderEdgeSet(src.border.right))  dest.border.right  = src.border.right;
	if (IsBorderEdgeSet(src.border.bottom)) dest.border.bottom = src.border.bottom;
	if (IsBorderEdgeSet(src.border.left))   dest.border.left   = src.border.left;
...
CD2DStyle CD2DControlUI::GetStatusStyle(ED2DStatus status) const
{
	CD2DStyle effective = m_defaultStyle;

	ApplyStyle(effective, GetStyle(status));
...
CD2DRadius CD2DControlUI::GetEffectiveRadius(const CD2DRadius& radius, const D2D1_RECT_F& borderRect) const
{
	...
	CD2DRadius result;
	result.topLeft     = convert(radius.topLeft);
	result.topRight    = convert(radius.topRight);
	result.bottomRight = convert(radius.bottomRight);
	result.bottomLeft  = convert(radius.bottomLeft);
	return result;
}
...
D2D1_RECT_F CD2DControlUI::GetBorderRectangle() const {
	...
	rect.left   -= style.border.left.width;
	rect.top    -= style.border.top.width;
	rect.right  += style.border.right.width;
	rect.bottom += style.border.bottom.width;
...
{
	float t = style.border.top.width;
	float r = style.border.right.width;
	float b = style.border.bottom.width;
	float l = style.border.left.width;
...
CD2DControlUI* CD2DControlUI::Clone() const
{
	return nullptr;
}
...
void CD2DControlUI::DrawControl(CD2DRender* pRender)
{
	...
	CD2DStyle style1;
	style1.border = CD2DBorderEdge(1.0f, Solid,  D2D1::ColorF(D2D1::ColorF::Red));
	//DrawBorder(style1, contentRect, pRender);
...
void CD2DControlUI::DrawBackground(const CD2DStyle& style, const D2D1_RECT_F& borderRect, CD2DRender* pRender)
{
	...
	if (!pRT || style.background.type == CD2DBackground::None)
		return;
...
void CD2DControlUI::DrawBorder(const CD2DStyle& style, const D2D1_RECT_F& borderRect, CD2DRender* pRender)
{
	...
	bool allSame = (IsBorderEdgeSet(style.border.top) &&
		IsBorderEdgeSet(style.border.right) &&
		IsBorderEdgeSet(style.border.bottom) &&
		IsBorderEdgeSet(style.border.left));
	if (!allSame)
	{
		// 获取内矩形（边框线条中心线位置）
	...
	else
	{
		...
		const CD2DBorderEdge& firstEdge = style.border.top;
		if (firstEdge.style == None) return;

		CComPtr<ID2D1SolidColorBrush> spBrush;
		pRT->CreateSolidColorBrush(firstEdge.color, &spBrush);
		if (!spBrush) return;

		CComPtr<ID2D1StrokeStyle> spStroke = pRender->CreateStrokeStyle(firstEdge.style);
		pRT->DrawGeometry(spGeom, spBrush, firstEdge.width, spStroke);
	}
}
// ========== 边框绘制 ==========
...
{
	ID2D1RenderTarget* pRT = pRender->GetRenderTarget();
	if (edge.style == None || edge.width <= 0.0f) return;

	CComPtr<ID2D1SolidColorBrush> spBrush;
	pRT->CreateSolidColorBrush(edge.color, &spBrush);
	if (!spBrush) return;

	CComPtr<ID2D1StrokeStyle> spStroke = pRender->CreateStrokeStyle(edge.style);
	pRT->DrawLine(start, end, spBrush, edge.width, spStroke);
}
...
void CD2DControlUI::DrawText(const CD2DStyle& style, const D2D1_RECT_F& contentRect, CD2DRender* pRender)
{
	...
	// 直接使用 CD2DRender 的统一布局创建方法
	CComPtr<IDWriteTextLayout> spLayout = pRender->CreateTextLayout(
		 m_text,
		style.text,
		GetWidth(),
		GetHeight());

	DWRITE_TEXT_METRICS metirc = {};
	spLayout->GetMetrics(&metirc);
	if (metirc.lineCount > 2)
	{
		int i = 0;
	}

	/*pRT->DrawTextLayout(D2D1::Point2F(contentRect.left, contentRect.top), spLayout, spBrush,
