...
void CD2DListUI::Layout()
{
	...
	if (needScroll)  // 只有需要滚动时才显示滚动条，获取其期望宽度
	{
		D2D1_SIZE_F barSize = m_scrollBar.GetDesiredSize(nullptr);
		scrollBarW = (barSize.width > 0) ? barSize.width : 10.0f; // 后备值
	}

	listWidth -= scrollBarW;
	...
	for (int i = 0; i < m_totalItemCount; ++i)
	{
		...
		CD2DStyle style = pItem->GetDefaultStyle();
		D2D1_SIZE_F size = GetStyleDecorationSize(style);
		float left = content.left + GetStyleDecorationLeft(style);
		y = y + GetStyleDecorationTop(style);

		pItem->SetPosition(left, y);
		pItem->SetWidthPolicy(CD2DSizePolicy::FixedSize(listWidth-size.width));
		pItem->SetHeightPolicy(CD2DSizePolicy::FixedSize(m_itemHeight - size.height));
		pItem->SetSize(listWidth-size.width, m_itemHeight - size.height);
	...
	if (needScroll)
	{
		D2D1_RECT_F barRect = D2D1::RectF(content.right - scrollBarW, content.top,
...
bool CD2DListUI::OnMouseDownL(const D2D1_POINT_2F& point)
{
	...
	if (idx >= 0)
	{
		CD2DControlUI* item = m_items[m_hoveredIndex];
		if (item) item->OnMouseDownL(contentPoint);
...
bool CD2DListUI::OnMouseMove(const D2D1_POINT_2F& point)
{
	...
	if (idx != -1)
	{
		CD2DControlUI* item = m_items[idx];
		if (item) item->OnMouseMove(contentPoint);
	}
...
bool CD2DListUI::OnMouseUpL(const D2D1_POINT_2F& point)
{
	...
	{
		CD2DControlUI* item = m_items[m_hoveredIndex];
		if (item) item->OnMouseUpL(contentPoint);
	}
...
{
	TriggerEvent(CD2DEvent::Checked);
}