	...
	, m_hoveredIndex(-1)
{
	...
	defaultStyle.thumb.background.type = CD2DBackground::Solid;
	defaultStyle.thumb.background.solid.color = D2D1::ColorF(0.6f, 0.6f, 0.6f);
	defaultStyle.thumb.radius = CD2DRadius(4.0f);
	defaultStyle.track.background.type = CD2DBackground::Solid;
	defaultStyle.track.background.solid.color = D2D1::ColorF(0.9f, 0.9f, 0.9f);
...
CAtlArray<CD2DControlUI*>& CD2DExploreUI::GetAllItems()
{
	return m_items;
}

const CAtlArray<CD2DControlUI*>& CD2DExploreUI::GetAllItems() const
{
	return m_items;
}
...
void CD2DExploreUI::Layout()
{
	...
	if (m_scrollBarVisible)
	{
		m_scrollBar.SetRectangle(D2D1::RectF(
			contentRect.right - scrollBarW, contentRect.top,
...
void CD2DExploreUI::LayoutWrap(const D2D1_RECT_F& viewport)
{
	...
	if (m_wrapAutoFit && m_itemWidth > 0.0f)
	{
		...
		for (int i = 0; i < (int)m_items.GetCount(); ++i)
		{
			...
			float ch = th - nc.height;   if (ch < 0) ch = 0;
			pItem->SetPosition(x+nc.width*0.5f, y+nc.height*0.5f);
			pItem->SetSize(cw, ch);
...
void CD2DExploreUI::DrawControl(CD2DRender* pRender)
{
	...
	D2D1_RECT_F clipRect = GetPaddingRectangle(); //暂时修改
	//clipRect.right -= (scrollBarW - m_hSpacing);
	...
	// 绘制拖拽幽灵控件位图（不受滚动偏移影响，因为它随鼠标移动，坐标为屏幕坐标）
	DrawDragGhost(pRender);

}

void CD2DExploreUI::DrawDragGhost(CD2DRender* pRender)
{
	if (!m_isDragging || m_dragSourceIndex < 0) return;
	ID2D1RenderTarget* pRT = pRender->GetRenderTarget();
	if (!pRT) return;

	// 如果位图还没创建，首次创建
	if (!m_pDragGhostBitmap)
	{
		CD2DControlUI* pSource = m_items.GetAt(m_dragSourceIndex);
		if (!pSource) return;
		// 记录源控件尺寸（用于后续绘制位图的大小）
		m_dragGhostSize = pSource->GetSize();
		CD2DStyle style = pSource->GetStatusStyle(ED2DStatus::Dragging);
		D2D1_SIZE_F decSize = GetStyleDecorationSize(style);
		m_dragGhostSize.height += decSize.height;
		m_dragGhostSize.width += decSize.width;

		CComPtr<ID2D1BitmapRenderTarget> pCompatRT;
		HRESULT hr = pRT->CreateCompatibleRenderTarget(m_dragGhostSize, &pCompatRT);
		if (FAILED(hr) || !pCompatRT) return;
		pRender->SetCompatibleRenderTarget(pCompatRT);
		
		// 暂时使源控件可见，以便绘制
		bool visible = pSource->IsVisible();
		D2D1_POINT_2F point = pSource->GetPosition();
		pSource->SetVisible(true);
		
		// 伪代码，实际需大量框架配合，这里只展示逻辑。
		pCompatRT->BeginDraw();
		pCompatRT->Clear(D2D1::ColorF(1.0,1.0,1.0,1.0));
		// 绘制控件
		pSource->SetPosition(GetStyleDecorationLeft(style), GetStyleDecorationTop(style));
		pSource->GetStatus().SetDragging(true);
		pSource->GetStatus().SetHover(false);
		pSource->DrawControl(pRender);
		pSource->GetStatus().SetDragging(false);

		pRender->SetCompatibleRenderTarget(nullptr);

		pCompatRT->EndDraw();
		pCompatRT->GetBitmap(&m_pDragGhostBitmap);
		pSource->SetVisible(visible); // 恢复可见性
		pSource->SetPosition(point.x, point.y);
	}
	...
	if (m_pDragGhostBitmap)
	{
		D2D1_RECT_F destRect = D2D1::RectF(
			m_dragGhostPos.x - m_dragGhostSize.width * 0.5f,
			m_dragGhostPos.y - m_dragGhostSize.height * 0.5f,
			m_dragGhostPos.x + m_dragGhostSize.width * 0.5f,
			m_dragGhostPos.y + m_dragGhostSize.height * 0.5f);
		pRT->DrawBitmap(m_pDragGhostBitmap, destRect, 0.8f, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR);
	}
}
...
{
	ClearDragOverState();
	m_pDragGhostBitmap.Release();   // ★ 释放位图
	// 恢复源控件可见
