...
// 移除控件
void CD2DContainerUI::DeleteChildren()
{
	// 使用带符号整数或确保 size_t 不会下溢
	for (int i = static_cast<int>(m_children.GetCount()) - 1; i >= 0; --i)
	{
		CD2DControlUI* pControl = m_children[i];  
		m_children.RemoveAt(i);
		delete pControl;
	}
}
...
D2D1_SIZE_F CD2DContainerUI::MeasureContent(CD2DRender* pRender, float maxWidth, float maxValueHeight) const
{
	...
	for (size_t i = 0; i < GetChildCount(); ++i)
	{
		...
		float rightDec = style.margin.right + style.border.right.width + style.padding.right;
		float botDec = style.margin.bottom + style.border.bottom.width + style.padding.bottom;
		float leftDec = style.margin.left + style.border.left.width + style.padding.left;
		float topDec = style.margin.top + style.border.top.width + style.padding.top;
...
bool CD2DContainerUI::OnMouseMove(const D2D1_POINT_2F& point)
{
	...
	{
		handled = m_scrollBar.OnMouseMove(point);
		m_scrollOffsetY = static_cast<float>(m_scrollBar.GetValue());
		return handled;
	}
	...
	if (pNewHover)
		return pNewHover->OnMouseMove(contentPt);
	CAtlString  String = GetText();
	//ZxDebugEx((__FUNCTION__"():: %ws \n", String));
	if(String == L"m_noteModalOverlay")
	{
		int xxxx = 0;
...
bool CD2DContainerUI::OnMouseLeave(const D2D1_POINT_2F& point)
{
	...
	auto handler = [](CD2DControlUI* ctrl, const D2D1_POINT_2F& pt) -> bool {
		return ctrl->OnMouseLeave(pt);
	};
	CD2DControlUI* target = DispatchEvent(contentPt, handler);
	if (target)
		return true;
...
D2D1_SIZE_F CD2DHBoxContainerUI::MeasureContent(CD2DRender* pRender, float maxWidth, float maxHeight) const
{
	...
	// 容器内容区域的实际可用尺寸
	float containerContentWidth = (maxWidth == FLT_MAX) 
		? (GetContentRectangle().right - GetContentRectangle().left) 
		: maxWidth;
	float containerContentHeight = (maxHeight == FLT_MAX) 
		? (GetContentRectangle().bottom - GetContentRectangle().top) 
		: maxHeight;
	...
	for (size_t i = 0; i < GetChildCount(); ++i)
	{
		...
		// 子控件内容最大可用宽度/高度
		float childMaxWidth = containerContentWidth - hDecor;
		if (childMaxWidth < 0.0f) childMaxWidth = 0.0f;
		float childMaxHeight = containerContentHeight - vDecor;
		if (childMaxHeight < 0.0f) childMaxHeight = 0.0f;

		D2D1_SIZE_F contentSize = pChild->GetDesiredSize(pRender, childMaxWidth, childMaxHeight);
...
void CD2DHBoxContainerUI::Layout1(CD2DRender* pRender)
{
	...
	if (bNeedMeasure)
	{
		...
		float containerLeftDec = containerStyle.margin.left + containerStyle.border.left.width + containerStyle.padding.left;
		float containerRightDec = containerStyle.margin.right + containerStyle.border.right.width + containerStyle.padding.right;
	...
	for (size_t i = 0; i < orderedItems.GetCount(); ++i)
	{
		...
		case CD2DAlignment::VCenter: 
			yVis = content.top + max(0.0f, (availH - item->visH) * 0.5f); 
			break;
...
D2D1_SIZE_F CD2DVBoxContainerUI::MeasureContent(CD2DRender* pRender, float maxWidth, float maxHeight) const
{
	...
	for (size_t i = 0; i < GetChildCount(); ++i)
	{
		...
		// 传入可用宽度（高度不限），让子控件正确换行并计算理想高度
		D2D1_SIZE_F contentSize = pChild->GetDesiredSize(pRender, childMaxWidth, FLT_MAX);
...
void CD2DVBoxContainerUI::Layout1(CD2DRender* pRender)
{
	...
	{
		CD2DStyle containerStyle = CalculateEffectiveStyle();
		float containerTopDec = containerStyle.margin.top + containerStyle.border.top.width + containerStyle.padding.top;
		float containerBotDec = containerStyle.margin.bottom + containerStyle.border.bottom.width + containerStyle.padding.bottom;
	...
	case CD2DAlignment::VTop:    
		...
		break;
	case CD2DAlignment::VCenter: 
		yVis = content.top + (availH - totalVisH - totalSpacing) * 0.5f;
		break;
	case CD2DAlignment::VBottom: 
		yVis = content.bottom - totalVisH - totalSpacing;
		break;
	}
